...
// ============================================================================
template <int HEAD_DIM_, int BC_, int WARPS_, int STAGES_, typename T_ = bf16>
struct KernelTraits {
    using Elem = T_;
...
__device__ inline void load_kv_tile(
    typename Traits::Elem* sK,   // ring bases (STAGES * BC * LD each)
    ...
    float Sacc[Traits::NC8][4])
{
    ...
    for (int n8 = 0; n8 < Traits::NC8; n8++) {
        ...
        for (int kt = 0; kt < Traits::KD; kt++) {
            unsigned b[2];
            astrai::ldmatrix_x2<typename Traits::Elem>(b,
                &sK[krow_l * Traits::LD
    ...
    float Oacc[Traits::DN8][4])
{
    ...
    for (int kt2 = 0; kt2 < Traits::KT2; kt2++) {
        ...
        for (int dn8 = 0; dn8 < Traits::DN8; dn8++) {
            unsigned b[2];
            astrai::ldmatrix_x2<typename Traits::Elem, true>(b,
                &sV[vrow_l * Traits::LD
